...
extern "C" int scanhash_verus(int thr_id, struct work *work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	if (version >= 7 && work->solution[5] > 0) {
		...
		memcpy(nonceSpace + 4, &pdata[EQNONCE_OFFSET - 3], 4);			// transfer the nonce values that would be in the header to
		memcpy(nonceSpace + 8, &pdata[EQNONCE_OFFSET + 1], 4);		// the 15 bytes available
		memcpy(nonceSpace + 12, &pdata[EQNONCE_OFFSET + 2], 1);
	}
	...
	do {
		...
		*hashes_done = nonce_buf + throughput;
		...
		if (work->nonces[0] != UINT32_MAX)
		{
			...
			((uint32_t *)(&nonceSpace[0]))[0] = work->nonces[0];
